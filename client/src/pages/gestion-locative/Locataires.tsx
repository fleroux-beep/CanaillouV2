...
interface LocataireGL { id: string; nom: string; prenom?: string; email?: string; telephone?: string; adresse?: string; siret?: string; notes?: string; }
const empty: Partial<LocataireGL> = { nom: "" };

export default function LocatairesGLPage() {
  const { data, create, update, remove, creating, updating, deleting } = useCrud<LocataireGL>("/api/gl/locataires", "Locataire");
  const [dialogOpen, setDialogOpen] = useState(false);
  const [editing, setEditing] = useState<LocataireGL | null>(null);
  const [form, setForm] = useState<Partial<LocataireGL>>(empty);
  ...
  return (
    <div className="space-y-6">
      <PageHeader title="Locataires" description="Locataires en gestion locative" actions={
      ...
      <DataTable data={data} columns={columns} searchKeys={["nom", "prenom", "email"]} searchPlaceholder="Rechercher..." emptyMessage="Aucun locataire" exportFileName="locataires-gl" />
