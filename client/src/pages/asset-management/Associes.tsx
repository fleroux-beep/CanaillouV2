...
interface Associe { id: string; nom: string; prenom?: string; email?: string; telephone?: string; adresse?: string; siret?: string; notes?: string; }
const empty: Partial<Associe> = { nom: "" };

export default function AssociesPage() {
  const { data, create, update, remove, creating, updating, deleting } = useCrud<Associe>("/api/am/associes", "Associe");
  const [dialogOpen, setDialogOpen] = useState(false);
  const [editing, setEditing] = useState<Associe | null>(null);
  const [form, setForm] = useState<Partial<Associe>>(empty);
  ...
  const handleSubmit = async (e: React.FormEvent) => { e.preventDefault(); if (editing) { await update({ ...form, id: editing.id } as Associe); } else { await create(form); } setDialogOpen(false); };
